...
function App(props) {
  ...
  const myMainnetDAIBalance = useContractReader(mainnetContracts, "DAI", "balanceOf", [
    "0x3746bc648a6868FAFFb333C9b356a0c1fF021719",
  ]);
  ...
  useEffect(() => {
    const updateCollectibles = async () => {
      ...
      for (let collectibleIndex = 0; collectibleIndex < numberCollectiblesCount; collectibleIndex++) {
        try {
          ...
          try {
            ...
            console.log("interface:", jsonManifest)
      ...
      console.log("collectibles:", collectiblesUpdate);
  ...
  */
  ...
  const [mutateAmount, setMutateAmount] = useState(0);
    ...
    <div className="App">
      ...
      <BrowserRouter>
        ...
        <Switch>
                    ...
                    <List.Item key={id + "_" + item.uri + "_" + item.owner}>
                      ...
                      <div>
                        ...
                        <Button
                          onClick={() => {
                            console.log("writeContracts", writeContracts);
                            tx(writeContracts.YourCollectible.safeTransferFrom(address, transferToAddresses[id], id, 1, [])).then((res) => {
                              window.location.reload();
                            });
                          }}
                        >
          ...
          <Route path="/apes">
            <br/>You can mutate your ape to bear if you have apes.<br/>
            {yourCollectibles && yourCollectibles[0]?.supply?.toNumber() > 0 &&
              <div style={{ width: 640, margin: "auto", marginTop: 32, paddingBottom: 32, fontSize: 25, fontWeight: 600}}>
                Number of apes you have : {yourCollectibles[0]?.owned?.toNumber()} <br/>
                <div style={{display:'flex'}}></div>
                  <Input
                    value={mutateAmount}
                    placeHolder="0"
                    onChange={e => {
                      setMutateAmount(e.target.value);
                    }}
                  ...
                  <Button
                    onClick={() => {
                      console.log("writeContracts", writeContracts);
                      tx(writeContracts.YourCollectible.mutateApeToBear(mutateAmount));
                    }}
                  >
                    Mutate
                  </Button>
